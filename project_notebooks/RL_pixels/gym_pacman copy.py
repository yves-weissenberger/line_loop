...
class Policy(nn.Module):
    def __init__(self,window_size,num_filter=2,size=2,pad=0,stride=2,lstm_hidden=0):
        ...
        self.conv1 = nn.Conv2d(3, self.num_filter, self.size,self.stride)
        self.conv2 = nn.Conv2d(self.num_filter, self.num_filter, self.size,self.stride)
        self.conv3 = nn.Conv2d(self.num_filter, self.num_filter, self.size,self.stride*2)
        #self.conv4 = init_(nn.Conv2d(self.num_filter, self.num_filter, self.size, self.stride, self.pad))

        self.affine1 = nn.Linear(72,70,bias=True)
        #self.dropout = nn.Dropout(p=0.1)


        self.lstm_hidden = lstm_hidden
        if self.lstm_hidden:
            self.lstm = nn.LSTMCell(input_size=70, hidden_size=self.lstm_hidden)
            self.h_1 = self.c_1 = torch.zeros([1,self.lstm_hidden])

            self.affine2 = nn.Linear(self.lstm_hidden, 4,bias=True)
        else:
            self.affine2 = nn.Linear(70, 4,bias=True)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.affine1(x.view([1,-1])))
...
def select_action(state):
    ...
    probs = policy(torch.Tensor(state).permute(2,0,1)[None,:,:,:])
    #print(probs)
    m = Categorical(probs)
    action = m.sample()
...
if __name__=='__main__':
    ...
    policy = Policy(window_size,lstm_hidden=0)
    policy.reset_lstm()
    ...
    while True:
        ...
        st = time.time()
        action = select_action(obs/255.)
        obs,rew,done,_ = env.step(action)

        #sys.stdout.write("\r{}".format(1/(time.time()-st)))
        #sys.stdout.flush()
        #print(rew)
        ...
        if done:
            ...
            avgR = alpha_*avgR + (1-alpha_)*np.sum(allR)
            print(np.sum(allR))
            allR = []
            episode_nr += 1
            epLen = 0
